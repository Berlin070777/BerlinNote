...
- 阅读进度：自动保存章节、句子、滚动位置和整本阅读百分比，下次打开同一本书会恢复。
- 句子收藏：每句旁边可以收藏/取消收藏，目录抽屉里可从收藏列表跳回原句。
...
- AI 音频缓存：同一句话、同一音色、同一语速只生成一次；后端落盘缓存，前端也会存入 IndexedDB，并在阅读栏显示音频来源。
